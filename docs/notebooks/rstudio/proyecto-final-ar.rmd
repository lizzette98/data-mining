...
Escribe la fecha actual (**NOTE-01**): 29 de mayo de 2021.
...
```{r}
NOMBRE_COMPLETO = "Lizzette Díaz"
DIGITOS_MATRICULA = 1748970
```
...
- ¿Cuál es la interpretación de la regla `{} => {diapers}` en la que se aprecia la ausencia del LHS? (**NOTE-19**):Que el item diapers no viene incluído en la lista.
- Calcula el valor de la métrica support para la regla `{beer} => {nuts}` (**NOTE-20**): 0 
- Calcula el valor de la métrica confidence para la regla `{beer} => {nuts}` (**NOTE-21**):0
- ¿Por qué no aparece listada la regla `{beer} => {nuts}`? (**NOTE-22**):Porque no hay relación entre cerveza y nueces; es decir, no hay reglas que asocien ambos ítems.
...
- ¿Cuál es el propósito de `session id`? (**NOTE-23**): Es la clave de la sesión y es igual para todos los datos de este dataset, pues todos estos datos están englobados dentro de la misma session id.
- ¿Cuál es el propósito de `track id`? (**NOTE-24**):Track id es diferente para cada uno de los datos, estos datos son los que son "limpiados" de la session id.
- ¿Consideras que las otras columnas del dataset son aplicables para análisis por medio de reglas de asociación, justifica tu respuesta? (**NOTE-25**): Sí, pues todos los datos están interrelacionados y hay reglas de asociación entre ellos ya sea en pequeña o gran medida.
...
- ¿Cuál es el ID y frecuencia absoluta de la sesión más frecuente? (**NOTE-26**): ID con más frecuencia: t_bacf06d3-9185-4183-84ea-ff0db51475ce, frecuencia: 865
- ¿Cuántos items tiene el itemset con mayor número de ocurrencias en la lista element length distribution? (**NOTE-27**): 1586
- En el contexto de este dataset con información de sesiones de streaming, ¿cuál es la interpretación de la media aritmética? (**NOTE-28**): 14.59
...
- ¿Qué factores están asociados con que el algoritmo `apriori` con parámetros por omisión no haya producido ninguna regla de asociación, justifica tu respuesta? (**NOTE-29**): Que la relación entre datos es muy baja o es inexistente, lo que ocasiona que no hayan reglas de asociación relacionadas.
- ¿Qué riesgos identificas con la asignación de valores muy bajos en los parámetros de  `support` y `confidence` al invocar el algoritmo `apriori`? Que no haya relación entre los datos, lo que ocasiona ausencia de reglas de asociación.
...
- Provee el enlace al video aquí: **NOTE-30** https://youtu.be/A9GMadUxcpg
